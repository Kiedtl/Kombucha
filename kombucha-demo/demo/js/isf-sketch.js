...
// SpaceIsThePlace by by mojovideotech
...
void main()
{
	...
	p.z += 0.003*TIME;
	float speed = rate * cos(TIME*0.02 + 3.1415926/4.0);
	vec3 dir=vec3(uv*zoom,1.);
	vec3 from=vec3(0.0, 0.0,0.0);
	vec3 forward = vec3(0.,0.,1.);
	float a1 = 3.1415926 * (move.x/RENDERSIZE.x-.5); // 0.9;
	mat2 rot1 = mat2(cos(a1),sin(a1),-sin(a1),cos(a1));
	float a2 = 3.1415926 * (move.y/RENDERSIZE.y-.5); .6;
	mat2 rot2 = mat2(cos(a2),sin(a2),-sin(a2),cos(a2));
	p.xz *= rot1;
	p.xy *= rot1;
	p.yz *= -rot2;
	p = abs(vec3(density)-mod(p,vec3(density*2.))); // tiling fold
	float t = field(p);
	...
	sampleShift /= depth; // make from 0 to 1
	float s=0.1;
	vec3 v=vec3(0.);
	for (int r=0; r<volsteps; r++) 
	{
		...
		{
			p=abs(p)/dot(p,p)-morph; // the magic formula
